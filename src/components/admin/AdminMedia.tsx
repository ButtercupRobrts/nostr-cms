...
function BrowseMediaSection() {
  ...
  const { data: blobs, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const isLocalRelay = selectedRelay.includes('relay.example.com') || selectedRelay.includes('localhost') || selectedRelay.includes('127.0.0.1');
  ...
  // Filter out non-media blobs (e.g., encrypted follow-list backups detected as text/plain).
  // Both ownerOptions and filteredBlobs derive from this so the owner dropdown doesn't
  // show users whose only blobs are non-media.
  const mediaBlobs = useMemo(() =>
    blobs?.filter(blob => !blob.type?.startsWith('text/plain')) ?? [],
    [blobs],
  );

  // Build list of unique owners from media blobs for the filter dropdown
  ...
  }, [mediaBlobs, mediaType, ownerFilter]);
