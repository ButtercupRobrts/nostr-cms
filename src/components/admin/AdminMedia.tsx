...
/** Validate that a URL uses a safe scheme (http/https) before rendering as an href */
...
function BrowseMediaSection() {
  ...
  const canDelete = isMaster || userRole === 'primary';
  ...
  const { data: blobs, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const isLocalRelay = isLocalSource(selectedRelay);
  ...
  const handleDeleteBlob = async (blob: BlossomBlob) => {
    ...
    try {
      ...
      const authorization = `Nostr ${toBase64(JSON.stringify(authEvent))}`;
                  ...
                  >
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                      ...
                      <Button variant="secondary" size="sm" className="w-full" asChild>
                        <a href={isSafeUrl(blob.url) ? blob.url : "#"} target="_blank" rel="noopener noreferrer">
                ...
                {pagedBlobs.map((blob, _index) => {
                  ...
                  return <div key={blob.sha256} className="flex items-center gap-4 px-4 py-2 hover:bg-muted/50 transition-colors rounded-md group">
                    <div className="flex-1 flex items-center gap-3 overflow-hidden">
                      ...
                      <a
                        href={isSafeUrl(blob.url) ? blob.url : "#"}
                        target="_blank"
