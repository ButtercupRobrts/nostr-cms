...
export function MarkdownToolbar({ textareaId, disabled }: MarkdownToolbarProps) {
  ...
  /**
   * Open the MediaSelectorDialog to pick an image from the library or upload
   * a new one. Saves the cursor position first so we can restore it after
   * the dialog closes (opening the dialog blurs the textarea).
   */
  ...
  const insertImageWithSize = useCallback((size: ImageSize) => {
    ...
    // Escape double quotes in URL and alt text to prevent attribute injection.
    const escUrl = pendingImage.url.replace(/"/g, '&quot;');
    const escAlt = pendingImage.alt.replace(/"/g, '&quot;');
    const insertText = size.width === '100%'
      ? `\n![${pendingImage.alt}](${pendingImage.url})\n`
      : `\n<img src="${escUrl}" alt="${escAlt}" style="width: ${size.width}; border-radius: var(--radius);" />\n`;
