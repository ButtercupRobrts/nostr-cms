 ...
 *
 * Per NIP-57, the zap receipt's own `pubkey` is the LNURL wallet service
 * (e.g. Alby), not the person who sent the zap. The zapper's pubkey is in:
 *   1. The `P` tag (uppercase) on the receipt — optional but fast to check.
 *   2. The `pubkey` field inside the `description` tag's JSON (the embedded
 *      kind 9734 zap request event) — MUST be present per NIP-57.
 *
 * Returns null if the event is not kind 9735, or if neither source yields
 * a valid 64-char hex pubkey.
 */
function getZapperPubkey(event: EventSummary): string | null {
  if (event.kind !== 9735) return null;

  // 1. Try the P tag (uppercase) — fast, no parsing
  const pTag = event.tags.find(t => t[0] === 'P' && t[1] && /^[0-9a-f]{64}$/.test(t[1]));
  if (pTag) return pTag[1];

  // 2. Parse the description tag (the embedded zap request event)
...
/**
 * Returns the pubkey that should be displayed as the "author" of an event.
 *
 * For kind 9735 zap receipts, this is the zapper (extracted from the P tag
 * or the embedded zap request), falling back to the event's own pubkey
 * (the wallet service) if the zapper can't be determined.
 *
 * For all other kinds, this is simply the event's pubkey.
 */
function getDisplayPubkey(event: EventSummary): string {
  if (event.kind === 9735) {
    return getZapperPubkey(event) ?? event.pubkey;
  }
  return event.pubkey;
}
...
function EventPreview({ event, onClose, onFilterAuthor }: {
  ...
  // For kind 9735 zap receipts, display the zapper (from P tag or embedded
  // zap request) instead of the wallet service that signed the receipt.
  // The raw event.pubkey is still shown in the Technical details section.
  const displayPubkey = getDisplayPubkey(event);
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex gap-2">
            <Button
              variant="outline"
              size="sm"
              onClick={() => { onFilterAuthor(displayPubkey); onClose(); }}
            >
                ...
                <TableBody>
                  {events.map((evt) => (
                    <TableRow key={evt.id} className="cursor-pointer hover:bg-muted/50" onClick={() => setSelectedEvent(evt)}>
                      ...
                      <TableCell>
                        <AuthorName pubkey={getDisplayPubkey(evt)} />
                      </TableCell>
