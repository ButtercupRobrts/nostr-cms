...
export function useDeleteBackup() {
  ...
  return useMutation({
    mutationFn: async (backup: FollowBackup) => {
      ...
      // If the backup uses Blossom storage, delete the blob first (non-fatal if it fails)
      if (backup.storage === 'blossom' && backup.blossomSha256) {
        await deleteBlossomBlob(user.signer, backup.blossomSha256, blossomServers);
      }

      // Publish kind 5 deletion to our relay (where the backup event is stored)
      await publishEvent.mutateAsync({
        event: {
          kind: 5,
          content: '',
...
async function createBackupEvent(
  ...
  for (const method of methodsToTry) {
    ...
    try {
      encrypted = await encryptFn(pubkey, plaintext);
      usedMethod = method;
      break;
    } catch (e) {
...
export async function decryptBackup(
  ...
  const decrypted = await decryptFn(pubkey, encrypted);
...
async function pruneOldBackups(
  ...
  for (const backup of toDelete) {
    // Delete Blossom blob first (non-fatal)
    if (backup.storage === 'blossom' && backup.blossomSha256) {
